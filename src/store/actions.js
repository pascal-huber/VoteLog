...
const actions = {
  ...
  async login(context, credentials) {
    const tokenData = await fetchToken(credentials.userName, credentials.password)
    ...
    persistSession(credentials.userName, credentials.webDav, tokenData.id_token, expiresAt)
  },
  async getData(context) {
    if (context.getters.isLoggedIn()) {
      try {
        ...
        const content = await client.getFileContents('/votelog_' + userName + '.json', {
          format: 'text',
        })
        ...
        if (error.status == '404') {
          ...
          console.log('hi <3')
          console.log('Let me create an empty file for you, just to ensure your WebDAV works.')
