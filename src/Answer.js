  ...
  function computePercentages() {
    for (var category in agreement) {
      for (var party in agreement[category]["parties"]) {
        agreement[category]["parties"][party] =
          Math.round(100 * (agreement[category]["parties"][party] / ctr[category]));
      }
      agreement[category]["swiss"] =
        Math.round(100 * (agreement[category]["swiss"] / ctr[category]));
    }
  }
    ...
    for (var iii = 0; iii < subjects[i].parties.length; iii++) {
      ...
      agreement[category]["parties"][partyVote.id] +=
        (userVote && userVote.answer != Answer.Novote) ?
          agreementValue(partyVote.answer, userVote?.answer, importance) : 0;
    }
    agreement[category]["swiss"] ??= 0;
    agreement[category]["swiss"] +=
      agreementValue(userVote?.answer, subjects[i].outcome, importance);
  }
}
