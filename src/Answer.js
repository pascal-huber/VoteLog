...
function agreementValue(otherAnswer, userAnswer, factor) {
    ...
    if (userAnswer == Answer.Abstention || otherAnswer == Answer.Abstention) return 0.5 * factor;
    return 0.0;
}
...
export function categoryAgreement(userVotes, subjects) {
    ...
    for (var i = 0; i < subjects.length; i++) {
        ...
        const categories = [...new Set(subjects[i].categories?.map((cat) => cat[0]))];
        ...
        for (var category in agreement) {
            for (var party in agreement[category]['parties']) {
                agreement[category]['parties'][party] = Math.round(
                    100 * (agreement[category]['parties'][party] / ctr[category])
                );
            }
            agreement[category]['swiss'] = Math.round(100 * (agreement[category]['swiss'] / ctr[category]));
        }
    }
    ...
    function updateAgreementCount(category, importance, userVote) {
        ...
        ctr[category] ??= 0;
        ctr[category] += userVote && userVote.answer != Answer.Novote ? importance : 0;
        for (var iii = 0; iii < subjects[i].parties.length; iii++) {
            ...
            agreement[category]['parties'][partyVote.id] +=
                userVote && userVote.answer != Answer.Novote
                    ? agreementValue(partyVote.answer, userVote?.answer, importance)
                    : 0;
        }
        agreement[category]['swiss'] ??= 0;
        agreement[category]['swiss'] += agreementValue(userVote?.answer, subjects[i].outcome, importance);
    }
}
...
export function agreement(subjects, userVotes, party = undefined) {
    ...
    var agreement = 0.0;
    for (var i = 0; i < subjects.length; i++) {
        const [sctr, sagreement] = agreementChange(subjects[i], userVotes, party);
    ...
    function agreementChange(subject, userVotes, party) {
        ...
        let otherAnswer = party ? subject.parties.find((p) => p.id == party)?.answer : subject.outcome;
        if (otherAnswer == undefined) return [0, 0];
        let importance = userVote.importance || 1;
        return [importance, agreementValue(otherAnswer, userVote.answer, importance)];
    }
}
